...
for num in range(0, 6):
    ...
    if num < 3:
        check = 1
        f = open("2.45km/muon_2D_def_E_" + str(num) + "_pos.txt", "r")
    else:
        check = 2
        f = open("2.45km/muon_2D_def_E_" + str(num - 3) + "_neg.txt", "r")
...
f = plt.imshow(array_phi_theta_flux[lowerlimit:upperlimit], extent=(np.amin(arrayphi2), np.amax(arrayphi2), np.amin(arraytheta2[lowerlimit:upperlimit]), np.amax(arraytheta2[lowerlimit:upperlimit])), aspect = 'auto')
plt.xlabel('phi')
plt.ylabel('cos theta')
plt.colorbar(f)
ax.set_title("total flux, normal B")

print(array_phi_theta_flux)

#plt.clim(0.9999, 1.0001)
...
Based on current energy model, the energy of muon gets higher than 10^23eV (10^11 TeV) when theta > 90deg

for theta < 90 deg, the maximum asymmetry, considering both positve muon and negative muons, is 

So for sam's plot, it is very unlikely that those asymmetry on theta > 90 is caused by cosmic ray muons

may be consider atmoshperical nutrino as well?

where does the lower part come from?
"""